	...
	<main id="main" class="main-site">
		...
		<div class="container">
			...
			<div class=" main-content-area">
					...
					<ul class="products-cart">
						...
						<li class="pr-cart-item">
							...
							<div class="product-name">
								<a class="link-to-product" href="{{route('product.details',['slug'=>$item->model->name])}}">{{$item->model->name}}</a>
							</div>
							...
							<div class="quantity">
								...
								<p class="text-center"><a href="" class="btn btn-info" wire:click.prevent="switchToSaveForLater('{{$item->rowId}}')">Save For Later</a></p>
							</div>
				...
				<div class="summary">
					<div class="order-summary">
						...
						@if(Session::has('coupon'))
							<p class="summary-info"><span class="title">Discount ({{Session::get('coupon')['code']}})</span><b class="index"
								>-${{number_format($discount)}} <a href="" wire:click.prevent="removeCoupon"><i class="fa fa-times-circle"></i></a></b></p>
							<p class="summary-info"><span class="title">Tax ({{config('cart.tax')}})</span><b class="index"
								>${{number_format($taxAfterDiscount)}}</b></p>
							<p class="summary-info"><span class="title">Subtotal With Discount</span><b class="index"
							>${{number_format($subtotalAfterDiscount)}}</b></p>
							<p class="summary-info"><span class="title">Total With Discount</span><b class="index"
							>${{number_format($totalAfterDiscount)}}</b></p>
						@else
								...
								<form  wire:submit.prevent="applyCouponCode">
									...
									@if(Session::has('coupon_message'))
										<div class="alert alert-danger" role="danger">
											<strong></strong> {{Session::get('coupon_message')}}
										</div>
									@endif
									<p class="row-in-form">
				...
				@endif
				<div class="wrap-iten-in-cart">
					<h3 class="title-box" style="border-bottom:1px solid;padding-bottom: 15px;">{{ Cart::instance('saveForLater')->count()}}  Saved For Later</h3>
					...
					<ul class="products-cart">
						...
						<li class="pr-cart-item">
							...
							<div class="product-name">
								<a class="link-to-product" href="{{route('product.details',['slug'=>$item->model->name])}}">{{$item->name}}</a>
							</div>
							<div class="price-field produtc-price"><p class="price">${{$item->model->regular_price}}</p></div>
							<div class="quantity">
								<div class="quantity-input">
								...
								<p class="text-center"><a href="" class="btn btn-success" wire:click.prevent="moveToCart('{{$item->rowId}}')">Move To Cart</a></p>
							</div>
